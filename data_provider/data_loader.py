...
class Dataset_ETT_hour(Dataset):
    ...
    def __getitem__(self, index):
        # ===== 修改开始：ETTh1 多变量预测直接使用滑动窗口索引，不再按 feat_id 拆成单变量样本 =====
        s_begin = index
        # ===== 修改结束：ETTh1 多变量预测直接使用滑动窗口索引，不再按 feat_id 拆成单变量样本 =====
        
        s_end = s_begin + self.seq_len
        r_begin = s_end - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
        # ===== 修改开始：ETTh1 返回所有变量列，使 batch_x 为 [B, seq_len, enc_in] 的真正多变量输入 =====
        seq_x = self.data_x[s_begin:s_end, :]
        seq_y = self.data_y[r_begin:r_end, :]
        # ===== 修改结束：ETTh1 返回所有变量列，使 batch_x 为 [B, seq_len, enc_in] 的真正多变量输入 =====
    ...
    def __len__(self):
        # ===== 修改开始：ETTh1 多变量样本数按滑动窗口计算，不再乘 enc_in 伪造多份单变量样本 =====
        return len(self.data_x) - self.seq_len - self.pred_len + 1
        # ===== 修改结束：ETTh1 多变量样本数按滑动窗口计算，不再乘 enc_in 伪造多份单变量样本 =====
...
class Dataset_ETT_minute(Dataset):
    ...
    def __getitem__(self, index):
        # ===== 修改开始：ETTm 多变量预测直接使用滑动窗口索引，不再按 feat_id 拆成单变量样本 =====
        s_begin = index
        # ===== 修改结束：ETTm 多变量预测直接使用滑动窗口索引，不再按 feat_id 拆成单变量样本 =====
        
        s_end = s_begin + self.seq_len
        r_begin = s_end - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
        # ===== 修改开始：ETTm 返回所有变量列，使 batch_x 为 [B, seq_len, enc_in] 的真正多变量输入 =====
        seq_x = self.data_x[s_begin:s_end, :]
        seq_y = self.data_y[r_begin:r_end, :]
        # ===== 修改结束：ETTm 返回所有变量列，使 batch_x 为 [B, seq_len, enc_in] 的真正多变量输入 =====
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]
    ...
    def __len__(self):
        # ===== 修改开始：ETTm 多变量样本数按真实滑动窗口计算，不再乘 enc_in 伪造单变量样本 =====
        return len(self.data_x) - self.seq_len - self.pred_len + 1
        # ===== 修改结束：ETTm 多变量样本数按真实滑动窗口计算，不再乘 enc_in 伪造单变量样本 =====
...
class Dataset_Custom(Dataset):
    ...
    def __getitem__(self, index):
        # ===== 修改开始：Exchange/custom 多变量预测直接使用滑动窗口索引，不再按 feat_id 拆成单变量样本 =====
        s_begin = index
        # ===== 修改结束：Exchange/custom 多变量预测直接使用滑动窗口索引，不再按 feat_id 拆成单变量样本 =====
        
        s_end = s_begin + self.seq_len
        r_begin = s_end - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
        # ===== 修改开始：Exchange/custom 返回所有变量列，使 batch_x 为 [B, seq_len, enc_in] 的真正多变量输入 =====
        seq_x = self.data_x[s_begin:s_end, :]
        seq_y = self.data_y[r_begin:r_end, :]
        # ===== 修改结束：Exchange/custom 返回所有变量列，使 batch_x 为 [B, seq_len, enc_in] 的真正多变量输入 =====
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]
    ...
    def __len__(self):
        # ===== 修改开始：Exchange/custom 多变量样本数按滑动窗口计算，不再乘 enc_in 伪造多份单变量样本 =====
        return len(self.data_x) - self.seq_len - self.pred_len + 1
        # ===== 修改结束：Exchange/custom 多变量样本数按滑动窗口计算，不再乘 enc_in 伪造多份单变量样本 =====
