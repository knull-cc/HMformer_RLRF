...
def test(model, test_data, test_loader, args, device, itr):
    maes, mses, rmses, mapes, mspes, smapes, nds = [],[],[],[],[],[],[]
    nn=[]
    model.eval()
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(test_loader)):
            ...
            rmses.append(rmse)
            smapes.append(smape)
    maes = [x * y for x, y in zip(maes, nn)]
    mses = [x * y for x, y in zip(mses, nn)]
    mae, mse= sum(maes) / sum(nn), sum(mses) / sum(nn)
