# ===== 修改开始：新增 ETTm2 多变量预测 HMformer 测试脚本，支持通过环境变量切换 loss 实验 =====
set -e
...
teacher_path_template=${TEACHER_PATH_TEMPLATE:-./checkpoints/ETTm2_96___PRED_LEN___feedback_dynamic_ema_sl336_ll48_pl__PRED_LEN___dm128_nh16_el2_gl3_df128_ebtimeF_itr0/checkpoint.pth}
...
# ===== 修改开始：打印脚本级实验配置，明确当前 ETTm2 实验选择 =====
echo "========== HMformer ETTm2 脚本配置 =========="
...
echo "============================================"
# ===== 修改结束：打印脚本级实验配置，明确当前 ETTm2 实验选择 =====
...
run_ettm2() {
  ...
  # ===== 修改开始：每个 horizon 开始前打印当前 ETTm2 运行任务，便于对应日志和结果 =====
  echo "---------- start ETTm2_96_${pred_len}_${exp_suffix} ----------"
  # ===== 修改结束：每个 horizon 开始前打印当前 ETTm2 运行任务，便于对应日志和结果 =====
  ...
  python -u main.py \
    ...
    --data_path ETTm2.csv \
    --model_id ETTm2_96_${pred_len}_${exp_suffix} \
    --model HMformer \
    --data ett_m \
    --features M \
    --freq t \
  ...
  # ===== 修改开始：每个 horizon 结束后打印完成提示，便于区分连续四个 ETTm2 实验 =====
  echo "---------- done ETTm2_96_${pred_len}_${exp_suffix} ----------"
  # ===== 修改结束：每个 horizon 结束后打印完成提示，便于区分连续四个 ETTm2 实验 =====
}
...
# ===== 修改结束：新增 ETTm2 多变量预测 HMformer 测试脚本，支持通过环境变量切换 loss 实验 =====
