# ===== 修改开始：新增 ETTh1 多变量预测 HMformer 测试脚本，支持通过环境变量切换 loss 实验 =====
set -e
...
# ===== 修改开始：打印脚本级实验配置，明确当前 ETTh1 实验选择 =====
echo "========== HMformer ETTh1 脚本配置 =========="
...
echo "============================================"
# ===== 修改结束：打印脚本级实验配置，明确当前 ETTh1 实验选择 =====
...
run_etth1() {
  ...
  # ===== 修改开始：每个 horizon 开始前打印当前 ETTh1 运行任务，便于对应日志和结果 =====
  echo "---------- start ETTh1_96_${pred_len}_${loss_mode} ----------"
  # ===== 修改结束：每个 horizon 开始前打印当前 ETTh1 运行任务，便于对应日志和结果 =====

  python -u main.py \
    --root_path ./dataset/ETT-small/ \
    --data_path ETTh1.csv \
    --model_id ETTh1_96_${pred_len}_${loss_mode} \
    --model HMformer \
    --data ett_h \
    --features M \
    --freq 0 \
  ...
  echo "---------- done ETTh1_96_${pred_len}_${loss_mode} ----------"
  # ===== 修改结束：每个 horizon 结束后打印完成提示，便于区分连续四个 ETTh1 实验 =====
}

run_etth1 96 256 256
run_etth1 192 256 256
run_etth1 336 512 512
run_etth1 720 512 512
# ===== 修改结束：新增 ETTh1 多变量预测 HMformer 测试脚本，支持通过环境变量切换 loss 实验 =====
